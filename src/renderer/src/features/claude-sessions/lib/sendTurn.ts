...
/**
 * Deliver a turn to an already-real (non-draft) session. Used by
 * `useComposerTarget`'s session/draft `send` branch so the composer's own
 * Send button and the queued-message flusher (`useQueuedMessageFlusher`)
 * share one code path instead of the flusher re-deriving "is this session
 * open" logic.
 *
 * Draft → real-session promotion is NOT handled here — a draft session is
 * never `running`, so it can never have a queued message to flush, and
 * `useComposerTarget`'s `send` keeps owning that branch.
 */
