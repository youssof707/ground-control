...
/**
 * Manage-prompt-shortcuts modal: a list of saved in-session shortcuts, each
 * deletable inline or clickable into an edit form. One `.modal-card`, list ↔
 * edit view swap in place (no stacked backdrops) — Escape steps back to the
 * list from edit, and closes the modal from the list.
 */
export function EditPromptShortcutsModal({
	open,
	onClose,
}: {
	open: boolean;
	...
	const remove = usePromptShortcutsStore((s) => s.remove);
	...
	useEffect(() => {
		if (!editingId) return;
		const current =
			usePromptShortcutsStore.getState().promptShortcuts[editingId];
		setForm(
			current
				? {
					title: current.title,
	...
	const handleSave = useCallback(async () => {
		...
		setError(null);
		try {
			const updated = await window.claude.updatePromptShortcut({
				id: view.id,
	...
	const handleDelete = useCallback(
		async (id: string) => {
			setError(null);
			try {
				await window.claude.deletePromptShortcut(id);
				remove(id);
			} catch (err) {
	...
	return (
		<div className="modal-backdrop" onClick={onClose} role="presentation">
			<div
				...
				role="dialog"
				aria-modal="true"
				aria-labelledby="edit-prompt-shortcuts-title"
				style={{ width: "min(480px, calc(100vw - 32px))" }}
			>
				{view.kind === "list" ? (
					<>
						<h2 id="edit-prompt-shortcuts-title" className="modal-title">
							Edit prompt shortcuts
						</h2>

						<Section title="Saved prompt shortcuts">
							{shortcuts.length > 0 ? (
								<div
									style={{
										display: "flex",
...
// Intentionally duplicated in EditShortcutsModal.tsx / AttachWorktreeModal.tsx —
...
const MODE_LABEL: Record<PromptShortcut["mode"], string> = {
	plan: "Plan",
	acceptEdits: "Accept edits",
};

function PromptShortcutRow({
	shortcut,
	onClick,
	onDelete,
}: {
	...
	return (
		...
		>
			...
			<DeletePromptShortcutButton onDelete={onDelete} />
		</div>
	);
}
...
/**
 * Two-step delete confirmation matching DeleteShortcutButton's pattern:
 * trash icon by default, click reveals a danger-token "Confirm delete?"
 * pill, second click within CONFIRM_REVERT_MS fires. Clicks stop bubbling
 * so hitting the trash doesn't also trigger the row's edit handler.
 */
function DeletePromptShortcutButton({
	onDelete,
}: {
	onDelete: () => Promise<void>;
}) {
	...
	if (deleting) {
		...
		// row doesn't reflow when we swap in the spinner.
		return (
			<button
				type="button"
				disabled
				aria-busy
				aria-label="Deleting prompt shortcut"
				style={{
					padding: "4px 10px",
					borderRadius: 6,
	...
	if (confirming) {
		return (
			<button
				type="button"
				onClick={handleClick}
				aria-label="Confirm delete prompt shortcut"
				style={{
					padding: "4px 10px",
					borderRadius: 6,
					border: `0.5px solid ${T.dangerBorder}`,
	...
	return (
		<button
			type="button"
			onClick={handleClick}
			aria-label="Delete prompt shortcut"
			style={{
				width: 24,
				height: 24,
				padding: 0,
